...
export const validateUserLogin = [body("login").isString().withMessage("Login must be a string").notEmpty().withMessage("Login is required"), body("password").isString().withMessage("Password must be a string").isLength({ min: 6 }).withMessage("Password must be at least 6 characters long")];
